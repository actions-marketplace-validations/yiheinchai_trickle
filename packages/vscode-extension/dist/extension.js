 ...
 */
function matchSequentialLayers(document, forwardCallLine, leafRecords) {
    const results = [];
    ...
    const expectedTypeCounts = new Map();
    ...
    if (!lastPassRecords) {
        ...
        for (const { layerType } of layerLines) {
            ...
            if (!group)
                return results; // Can't match — bail
        }
        // Build by occurrence order: for each type, pick the last N records
        const typePickCounters = new Map();
        const typeLatest = new Map();
        for (const [type, count] of expectedTypeCounts) {
    ...
    provideInlayHints(document, range) {
        ...
        if (document.uri.scheme === 'file') {
            ...
            if (actLines) {
                ...
                const buildActivationLabel = (ac, includeShape) => {
                ...
                const buildActivationTooltip = (ac) => {
                    const tooltipLines = [
                ...
                const sequentialHintLines = new Set();
                    ...
                    if (leafRecords.length > 1) {
                        ...
                        if (sequentialMatches.length > 0) {
                            for (const { layerLineNo, record } of sequentialMatches) {
                                if (layerLineNo - 1 < range.start.line || layerLineNo - 1 > range.end.line)
                                    continue;
