...
def _sanitize(value: Any, depth: int = 2) -> Any:
    ...
    # But for scalar values (0-d tensors, numpy scalars), show the actual value
...
def _generate_setup_code(filename: str, module_name: str, trace_vars: bool) -> str:
    ...
    # Dimension labels: when code does B, T, C = x.size(), record the names
    if trace_vars:
        lines.extend([
            ...
            f"        _r = {{'kind': 'dim_labels', 'varName': _var, 'labels': _names, 'line': _line, 'file': {filename!r}}}",
            "        if _func: _r['funcName'] = _func",
...
def _make_dim_label_stmt(node: ast.AST, func_name: str | None = None) -> ast.Expr | None:
    ...
    This captures the mapping from tensor variable `x` to dimension names ["A", "B", "C"],
    so the VSCode extension can display shapes as Tensor[A=2, B=16, C=64].
    """
    ...
    # Target must be a tuple of simple names: (A, B, C) or A, B, C
    ...
    for elt in target.elts:
        if isinstance(elt, ast.Name):
            ...
            return None  # Complex unpacking, skip

    # Value must be x.size() or x.size(dim) or x.shape
    tensor_expr = None
    if isinstance(value, ast.Call):
        # x.size() or x.size(0) — we only care about no-arg .size()
    ...
    # Get the tensor variable name for the record
    ...
    # Build: _trickle_dl(x, ["A", "B", "C"], "x", line, func_name)
    args: list[ast.expr] = [
        tensor_expr,  # The actual tensor value (to check if it has .shape)
...
def _expr_to_name(expr: ast.expr) -> str | None:
    """Convert an AST expression to a display name string, or None if too complex."""
