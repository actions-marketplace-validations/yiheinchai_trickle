...
/**
 * Generate a TypeScript class declaration from observed methods.
 */
function generateTsClassStub(clsName: string, methods: FunctionTypeData[]): string {
  const extracted: ExtractedInterface[] = [];
  ...
  for (const fn of methods) {
    ...
    // Build parameter list (skip 'this'/'self')
    const params: string[] = [];
    if (fn.argsType.kind === "tuple") {
      for (let i = 0; i < (fn.argsType.elements || []).length; i++) {
        const pname = paramNames[i] || `arg${i}`;
        if (pname === "self" || pname === "this") continue;
        const el = fn.argsType.elements![i];
        const { isOptional, innerType } = extractOptional(el);
        if (isOptional) {
          params.push(`${pname}?: ${typeNodeToTS(innerType, extracted, toPascalCase(clsName), pname, 0)}`);
        } else {
          params.push(`${pname}: ${typeNodeToTS(el, extracted, toPascalCase(clsName), pname, 0)}`);
        }
      }
    }

    const retType = typeNodeToTS(fn.returnType, extracted, toPascalCase(clsName), undefined, 0);
    const returnTypeStr = fn.isAsync ? `Promise<${retType}>` : retType;
    methodLines.push(`  ${methodName}(${params.join(", ")}): ${returnTypeStr};`);
  }
  ...
  while (cursor < extracted.length) {
    ...
    if (emitted.has(iface.name)) continue;
    emitted.add(iface.name);
    preSections.push(renderInterface(iface.name, iface.node, extracted));
    preSections.push("");
  }

  const classBody = methodLines.length > 0 ? methodLines.join("\n") : "  // no methods observed";
  const classBlock = `export declare class ${clsName} {\n${classBody}\n}`;
...
export function generateFromJsonl(jsonlPath: string): Record<string, { ts: string; python: string }> {
  ...
  for (const [mod, fns] of byModule) {
    // TypeScript — group class methods together
    ...
    // Separate class methods from standalone functions for TS too
    const tsClassMethods = new Map<string, FunctionTypeData[]>();
    const tsStandaloneFns: FunctionTypeData[] = [];
    for (const fn of fns) {
      if (fn.name.includes(".")) {
        const clsName = fn.name.split(".")[0];
        if (!tsClassMethods.has(clsName)) tsClassMethods.set(clsName, []);
        tsClassMethods.get(clsName)!.push(fn);
      } else {
        tsStandaloneFns.push(fn);
      }
    }

    // Generate TS class declarations
    for (const [clsName, methods] of tsClassMethods) {
      tsSections.push(generateTsClassStub(clsName, methods));
      tsSections.push("");
    }

    // Generate standalone TS functions
    for (const fn of tsStandaloneFns) {
