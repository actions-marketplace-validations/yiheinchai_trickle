...
1. **Agent security observability** — The "Lethal Trifecta" (prompt injection + tool abuse + data exfiltration) is a live threat (OpenClaw vulns March 14, OWASP AI Agent Security Cheat Sheet published). Build `trickle security` that scans captured traces for: (a) prompt injection patterns in LLM inputs, (b) unauthorized or unexpected tool calls (tool called that wasn't in the agent's declared toolset), (c) data exfiltration signals (PII/secrets in LLM outputs or tool responses), (d) privilege escalation (agent accessing tools beyond its scope). Trickle already captures all tool calls and LLM I/O — this is pattern matching on existing data. No competitor offers local-first agent security scanning.

2. **Compliance audit export** — EU AI Act applies Aug 2, 2026 (4.5 months). Colorado AI Act June 30, 2026 (3.5 months). Both require audit trails for AI decision-making. Build `trickle audit` that exports trickle's JSONL data as structured compliance reports: decision lineage (which LLM call led to which tool call led to which output), human-in-the-loop checkpoints, risk classifications, and timestamped event logs. Trickle's local-first design means sensitive audit data never leaves the developer's machine — a unique selling point for regulated industries.
