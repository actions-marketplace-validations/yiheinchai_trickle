...
- [Zero-Config Auto-Typing (`trickle/auto`)](#zero-config-auto-typing-trickleauto)
...
## Zero-Config Auto-Typing (`trickle/auto`)

The simplest way to use trickle: add **one line** to your app and types appear automatically. No CLI, no backend, no configuration.

### Usage

```js
// app.js — add this ONE LINE at the top:
require('trickle/auto');

// ... your normal code below
const utils = require('./utils');
utils.calculateDiscount(99.99, 15);
```
...
That's it. A `.d.ts` file appears next to each source file with types for every function that was called.

### What it does

When you `require('trickle/auto')`, trickle:

1. **Forces local mode** — no backend needed, observations go to `.trickle/observations.jsonl`
2. **Instruments all functions** — patches `Module._compile` to wrap every function in files loaded after the require
3. **Generates types in the background** — a timer regenerates `.d.ts` files every 3 seconds as new types are captured
...
function calculateDiscount(price, percentage) {
  return { original: price, discount: price * (percentage / 100), final: price - discount };
}
calculateDiscount(99.99, 15);
```

trickle/auto generates:
```ts
...
- Functions must be in **required modules** (not the entry file itself) for auto-instrumentation to work. This is because the entry file is already being compiled when `require('trickle/auto')` runs. In practice, this isn't an issue since real apps load modules via `require()`.
