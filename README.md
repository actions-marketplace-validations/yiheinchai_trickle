...
## Zero-Config Auto-Typing (`trickle/auto` / `trickle.auto`)

The simplest way to use trickle: add **one line** to your app and types appear automatically. No CLI, no backend, no configuration. Works for both **JavaScript** and **Python**.
...
```python
# app.py — add this ONE LINE at the top:
import trickle.auto

# ... your normal code below
from my_utils import calculate_discount, format_invoice
calculate_discount(99.99, 15)
```

```bash
python app.py
# [trickle.auto] 3 function type(s) written to .pyi
```
...
```python
class CalculateDiscountOutput(TypedDict):
    original: float
    discount: float
    final: float
